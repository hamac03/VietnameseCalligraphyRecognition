...
def _get_databaunch(config):
    ...
    print("*"*80)
    print(f"Using: {config.dataset_scheme}")
...
if __name__ == "__main__":
    ...
    if config.model_checkpoint:
        ...
        classifier_weight_shape = pretrained_state_dict['net']['module.decoder.classifier.weight'].shape
        model_weight_shape = model.module.decoder.classifier.weight.shape
        if any([classifier_weight_shape[i] != model_weight_shape[i] for i in range(len(classifier_weight_shape))]):
            print(f'change classifier weight shape from {classifier_weight_shape} to {model_weight_shape}')
            del pretrained_state_dict['net']['module.decoder.trg_word_emb.weight']
            del pretrained_state_dict['net']['module.decoder.classifier.weight']
            del pretrained_state_dict['net']['module.decoder.classifier.bias']
        else:
            print(f'keep classifier weight shape from {classifier_weight_shape} to {model_weight_shape}')
        model.load_state_dict(pretrained_state_dict['net'], strict=False)
    ...
    if iteration == 0:
        ...
        logging.info(f'Resume training from iteration {iteration}.')
    # print(f'continue to train:{iteration}')
    start_time = time.time()
    """ start training """
    best_accuracy = 0.
    # training loop
    # print(f"iteration: {iteration}->{config.training_epochs * config.iter_num}")
    # print(f"total iteration: {int(config.training_epochs * config.iter_num)}")
    # print(f"lr schedule {len(lr_schedule)}")
    for iteration in tqdm(
        ...
        if iteration % config.training_show_iters == 0:
            # IndexError: index 45 is out of bounds for dimension 0 with size 20 ## 20 is the last batch 
            i = random.randint(0, min(config.dataset_train_batch_size - 1,len(image_tensors)))
            ...
            logging.info(f'iteration:{iteration}--> lr: {lr}--> train loss:{train_loss_avg.val()}')
            ...
            overlaps = []
            ABI_scores = attn.mean(1).reshape(-1, 40, 8, 32)
            # ABI_scores = attn.mean(1).reshape(-1, 25, 8, 32) #25 dành cho chữ thường, 40 cho stroke, 125 cho arttext
            # ABI_scores = attn.mean(1).reshape(-1, 125, 8, 32)
        ...
        if iteration % config.training_save_iters == 0 and iteration!=140000:
            checkpoint = {
                'net': model.state_dict(),
                "optimizer": optimizer.state_dict(),
                'iteration': iteration,
        ...
        # Save and eval at last
        if iteration == int(config.training_epochs * config.iter_num) - 1:
            checkpoint = {
                'net': model.state_dict(),
                "optimizer": optimizer.state_dict(),
                'iteration': iteration,
            ...
            torch.save(checkpoint, f'./saved_models/{config.global_name}/last.pth')
            logging.info('eval model at last')
            elapsed_time = time.time() - start_time
            model.eval()
            eval_acc_words = 0.
            eval_acc = 0.
