...
from Dino.decoder.transformer_layers import TFDecoderLayer
from Dino.decoder.transformer_module import PositionalEncoding
from Dino.decoder.base_decoder import BaseDecoder
...
class NRTRDecoder(BaseDecoder):
    ...
    def forward_train(self, feat, out_enc, targets_dict, img_metas):
        ...
        attn_output, attn = self._attention(targets, out_enc, src_mask=src_mask)
    ...
    def forward_test(self, feat, out_enc, img_metas):
        ...
        for step in range(0, self.max_seq_len):
            decoder_output, attn = self._attention(
    ...
    def forward_test_speed(self, feat, out_enc, img_metas):
        ...
        for step in range(0, self.max_seq_len):
            ...
            _, step_max_index = torch.max(step_result, dim=-1)
            init_target_seq[:, step + 1] = step_max_index
            if step_result.argmax() == 224: # old is 91 check ImageDataset
                break
