...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, test_dataloader in enumerate(test_dataloaders):
            ...
            evaluation_log += f"dataset: {eval_data_name[i]} --> word_num: {res['words']} --> accuracy: {res['cwr']} --> CER: {1-res['ccr']}"
            evaluation_log += '\n'
        mean_loss = eval_acc / eval_acc_words
        evaluation_log += f"total_accuracy: {mean_loss}"
        print(evaluation_log + '\n')
        with open(os.path.join(config.global_workdir, 'evaluation.txt'), 'a') as f:
            f.write(evaluation_log + '\n')

        print(f"Total time: {total_time}")
        print(f"time per image: {total_time / eval_acc_words}")