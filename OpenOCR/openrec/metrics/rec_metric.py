...
class RecMetric(object):
    ...
    def eval_metric(self, pred_label, *args, **kwargs):
        ...
        for (pred, pred_conf), (target, _) in zip(preds, labels):
            ...
            if self.is_lower:
                pred = pred.lower()
                target = target.lower()
            raw_distance = Levenshtein.distance(pred, target)
            self.char_distance += raw_distance
            self.total_ref_chars += len(target)
    ...
    def get_metric(self, training=False):
        ...
        cer = self.char_distance / (self.total_ref_chars + self.eps)
        num_samples = self.all_num
        time_per_image = (self.total_inference_time / (self.all_num + self.eps))
        self.reset()
        return {
            'acc': acc,
