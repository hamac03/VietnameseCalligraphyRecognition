...
class Trainer(object):
    ...
    def __init__(self, cfg, mode='train'):
        ...
        # Load Vietnamese character mappings
        with open('Vietnamese-Characters-Simplified.txt', 'r', encoding='utf8') as f:
            for line in f:
                char, code = line.strip().split()
                self.char_to_code[char] = code
                self.code_to_char[code] = char
        
        # Load dictionary with strokes
        with open('dict-with-strokes.txt', 'r', encoding='utf8') as f:
            for line in f:
                word, strokes = line.strip().split(' ')
                self.word_to_strokes[strokes] = word
        
        print(self.word_to_strokes)
    ...
    def encode_text(self, text: str) -> str:
        """Convert text to stroke codes"""
        encoded = []
        for char in text:
            code = self.char_to_code.get(char, char)
            encoded.append(code)
        return '-'.join(encoded)

    def decode_prediction(self, encoded_text: str) -> str:
        """Convert stroke codes back to text"""
        # First try to find the complete word in dictionary
        if encoded_text in self.word_to_strokes:
            return self.word_to_strokes[encoded_text]
        
        # If not found, decode character by character
        decoded = []
        codes = encoded_text.split('-')
        for code in codes:
            char = self.code_to_char.get(code, code)
            decoded.append(char)
        return ''.join(decoded)
    ...
    def contrastive_loss(self, features, labels):
        ...
        # Calculate mean of positive pair losses
        ...
        for epoch in range(start_epoch, epoch_num + 1):
            ...
            for idx, batch in enumerate(self.train_dataloader):
                if isinstance(batch[0], str):
                    batch[0] = self.encode_text(batch[0])
                ...
                if self.scaler:
                    with torch.cuda.amp.autocast():
                        ...
                        outputs, features = self.model(batch[0], return_features=True)
                        
                        # Calculate appearance loss
                        appearance_loss = self.loss_class(outputs, batch[1])
                        
                        # Calculate contrastive loss
                        contrastive_loss = self.contrastive_loss(features, batch[1])
                        
                        # Combine losses with 1:1 ratio
                        total_loss = appearance_loss + contrastive_loss
                ...
                if self.local_rank == 0 and self.use_ema and epoch > (
                    ...
                    with torch.no_grad():
                        loss_currn = total_loss.detach().cpu().numpy().mean()
                        loss_avg = ((loss_avg *
