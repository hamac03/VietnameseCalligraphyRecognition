...
def compute_accuracy(ground_truth, predictions, mode="full_sequence"):
    ...
    if mode == "per_char":
        ...
        # Tính CER (Character Error Rate) bằng Levenshtein distance
        total_chars = 0
        total_errors = 0
        for gt, pred in zip(ground_truth, predictions):
            total_chars += len(gt)
            # Tính Levenshtein distance giữa nhãn thật và dự đoán
            errors = levenshtein_distance(gt, pred)
            total_errors += errors
        
        # CER = (Tổng số lỗi / Tổng số ký tự trong nhãn thật)
        cer = total_errors / total_chars if total_chars > 0 else 0
        avg_accuracy = cer
    elif mode == 'avg_time':  # Tính thời gian trung bình dự đoán mỗi ảnh
        total_time = 0
        for img in predictions:
            start_time = time.time()  # Lấy thời gian bắt đầu dự đoán
            # Giả sử bạn có hàm dự đoán cụ thể, ví dụ như model.predict(img)
            # model.predict(img)
            end_time = time.time()  # Lấy thời gian kết thúc dự đoán
            total_time += (end_time - start_time)
        
        avg_time_per_image = total_time / len(predictions) if len(predictions) > 0 else 0
        avg_accuracy = avg_time_per_image
    else:
