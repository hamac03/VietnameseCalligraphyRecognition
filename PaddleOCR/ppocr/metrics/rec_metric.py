...
class RecMetric(object):
    ...
    def __call__(self, pred_label, *args, **kwargs):
        ...
        all_num = 0
        norm_edit_dis = 0.0
        total_cer = 0.0  # Initialize CER sum
        ...
        for (pred, pred_conf), (target, _) in zip(preds, labels):
            ...
            cer = Levenshtein.distance(pred, target) / (len(target) + self.eps)
            total_cer += cer
        ...
        self.all_num += all_num
        self.norm_edit_dis += norm_edit_dis
        self.total_cer += total_cer  # Accumulate total CER

        return {
            "acc": correct_num / (all_num + self.eps),
            "norm_edit_dis": 1 - norm_edit_dis / (all_num + self.eps),
            "cer": total_cer / (all_num + self.eps),  # Average CER
        }
    ...
    def get_metric(self):
        ...
        {
            'acc': Accuracy,
            'norm_edit_dis': Normalized Edit Distance,
            'cer': Character Error Rate,
        }
        """
        acc = self.correct_num / (self.all_num + self.eps)
        norm_edit_dis = 1 - self.norm_edit_dis / (self.all_num + self.eps)
        cer = self.total_cer / (self.all_num + self.eps)  # Calculate average CER

        # Reset metrics after retrieving
        self.reset()
    ...
    def reset(self):
        ...
        self.all_num = 0
        self.norm_edit_dis = 0
        self.total_cer = 0.0  # Reset total CER
