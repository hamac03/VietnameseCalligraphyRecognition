...
def eval(
    ...
    with paddle.no_grad():
        ...
        for idx, batch in enumerate(valid_dataloader):
            if idx >= max_iter:
                break
            # print(f"batch{idx}: {batch[1].shape}")
            # print(f"batch{idx}: {batch[1]}")
            ...
            # print(preds)
            # post_preds = post_process_class(preds)
            # post_gt = post_process_class(batch[1])
            # print(f"post_pred: {post_preds}")
            # print(f"post_gt: {post_gt}")


            batch_numpy = []
            # print(f"pred: {all_preds}")
            # print(f"all_groundtruths: {all_groundtruths}")
            for item in batch:
            ...
            if model_type in ["table", "kie"]:
                ...
                print(f"post preds: {post_result[0]}")
                print(f"post gt   : {post_result[1]}")
    ...
    pbar.close()
    model.train()
    metric["time_per_image"] = total_time / total_frame 
    # print(f"Metric: {metric}")
    return metric
